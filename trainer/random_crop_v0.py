...
class RandomResizedCropAroundTarget(nn.Module):
    ...
    def apply_transform(self, input: torch.Tensor, params: Dict[str, torch.Tensor]) -> torch.Tensor:
        # Kornia 0.7.3 没有 apply_crop，使用 crop_and_resize 替代
        # input: (B, C, H, W)
        # params['src']: (B, 4, 2) 源box四点
        # params['dst']: (B, 4, 2) 目标box四点
        # self.size: (h, w)
        # crop_and_resize 需要 (B, C, H, W), boxes (B, 4, 2), size (h, w)
        out_size = self.size  # (h, w)
        return crop_and_resize(input, params['src'], out_size)
